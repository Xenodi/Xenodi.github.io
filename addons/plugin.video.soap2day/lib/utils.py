...
class Listing:
    ...
    def addItem(self, label, image=ADDON_ICON, data={}, color=None, infoType=None, infoLabels=None, isFolder=True, **properties):
        if color == None:
            list_item = xbmcgui.ListItem(label, label2=label)
        else:
...
def KeyboardInput(heading, inputHidden=False):
    kb = xbmc.Keyboard("default", "heading", True)
    kb.setDefault("")
    kb.setHeading(heading)
    kb.setHiddenInput(inputHidden)
    kb.doModal()
